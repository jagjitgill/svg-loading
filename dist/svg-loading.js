...
function pulseLoading(cssSelector) {
    var button = document.querySelector(cssSelector),
        ...
        loading = new LoadingButton(button, options);
        addClass(button, 'pulse-loading');

    function pulseAnimation(segment) {
        var self = this;
        segment.draw('100%', '150%', 1.8, {
            circular: true,
            callback: function() {
                if (self.infinite) {
                    pulseAnimation.call(self, segment);
                } else {
                    segment.draw('100%', '100% + 0.1', 1.3, {
                        circular: true,
                        callback: function() {
                            self.handleResponse();
                        }
    ...
    if (!cssSelector || document.querySelector(cssSelector) == null) {
        ...
        switch (options.type.toLowerCase()) {
            ...
            case 'infinity':
                ...
                break;
            case 'pulse':
                loading = pulseLoading(cssSelector);
                break;
